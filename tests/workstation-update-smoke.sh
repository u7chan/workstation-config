...
# Normal path: a clean full run where all steps succeed.
prepare_run normal
run_update
...
grep -Fq '1/2 update-ai' <<<"$normal_watch"
grep -Fq '2/2 mise upgrade herdr' <<<"$normal_watch"
...
# An empty AI tool selection is an explicit successful skip.
...
source_interactive_shell
grep -Fq '⟳ 非同期で更新中です...' "$shell_output"

write_shell_state running '' stale
source_interactive_shell
grep -Fq '✗ workstation update stopped unexpectedly — watch-update' "$shell_output"

write_shell_state failed update-ai
source_interactive_shell
grep -Fq '✗ update-ai update failed — watch-update --verbose' "$shell_output"
