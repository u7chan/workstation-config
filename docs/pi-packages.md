...
|---|---|---|---|---|---|---|---|
| `npm:pi-web-access` | Web access。`web_search`、`fetch_content`、`get_search_content`、`source_check` | `nicobailon/pi-web-access` / 登録はPi Package managerが所有 | `personal`でPiを選択した場合のglobal user package。`bootstrap`、`myupdate`、`update-ai --pi`から更新 | 共通要件（Node `22.19+` / Pi `0.84.2+`） | 非機密デフォルト（`~/.pi/agent/web-search.json` の `workflow: "auto-summary"`）だけはchezmoiが`create`属性でPi選択時のみ配布（sourceは`home/dot_pi/agent/create_web-search.json`）。pi-web-access 0.29.0でpiのagent dirへ設定パスが変更されたため、旧`~/.pi/web-search.json`が残っていて新パスにファイルが無ければbootstrapが配布前に新パスへ移動する（新旧両方がある場合は移行せず、旧ファイルの内容は読まれない）。既存ファイルは上書きしないため、API key等はユーザーが手動追加（runtime）。OAuth、cookie、cacheはruntimeで管理 | `settings.json`の`npmCommand`からSafe-chain npmを使用。conversionの`web_run`は公開せず、Web機能は本Packageに委譲 | `./tests/pi-packages-smoke.sh`、`./tests/pi-web-search-smoke.sh`、Pi起動後の4 toolの登録、zero-config検索を確認 |
