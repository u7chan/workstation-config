...
| `npm:pi-codex-image-gen` | Codex loginを使う画像生成・編集。`codex_generate_image` | `jvm/pi-mono`の`packages/pi-codex-image-gen` / Pi Package managerが登録を所有 | `personal`かつPi選択時のglobal user package。`bootstrap`、`myupdate`、`update-ai --pi` | Package固有のNode `20.6+`。共通要件はNode `22.19+` / Pi `0.84.2+` | `~/.pi/agent/extensions/codex-image-gen.json`はbootstrapで作成しない。global出力は`~/.pi/agent/generated-images/`、project出力は`.pi/generated-images/`。生成画像とauth stateはGit管理しない | Safe-chain npm経路。conversionの`imagegen`を公開せず、画像生成・編集は本Packageに委譲 | `./tests/pi-packages-smoke.sh`、tool登録、必要時に実画像生成と有効な出力形式・tracked files無変更を確認 |
| `npm:@howaboua/pi-codex-conversion` | Codex/GPT向けadapter。`exec_command`、`write_stdin`、`apply_patch`、`view_image`、prompt/tool adaptation | `IgorWarzocha/howaboua-pi-stuff`の`packages/pi-codex-conversion` / Pi Package managerが登録を所有 | `personal`かつPi選択時のglobal user package。`bootstrap`、`myupdate`、`update-ai --pi` | Node `22.19+`。Pi関連peer dependencyは`0.84.2+`。x64/arm64 Linux native helperを含む | `~/.pi/agent/pi-codex-conversion.json`のうち`tools.webRun`、`tools.imageGeneration`、`tools.webRunOnly`、`tools.imageGenerationOnly`の4キーだけを`update-ai`が管理。その他のキーは保持 | Safe-chain npm経路。4キーをfalseにしてconversionの`web_run` / `imagegen`を無効化し、Webは`pi-web-access`、画像生成は`pi-codex-image-gen`へ委譲。`view_image`は画像入力のため維持 | `./tests/pi-packages-smoke.sh`、Pi `0.84.1`拒否 / `0.84.2`受理の境界、Codex sessionでnative helperとtool surfaceを確認 |
